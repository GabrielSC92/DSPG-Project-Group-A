...
def generate_demo_data(n_rows: int = 200) -> pd.DataFrame:
    ...
    data = {
        ...
        "Correlation": [round(random.uniform(0.5, 1.0), 3) for _ in range(n_rows)],
...
colors = ["#FF6B35", "#3B82F6", "#8B5CF6", "#10B981", "#F59E0B", "#EF4444", "#06B6D4", "#EC4899"]
...
if chart_type == "Time Series":
    ...
    if group_by in ["Topic", "Source"]:
        agg_df = filtered_df.groupby(["Period", group_by])[metric].mean().reset_index()
        fig = px.line(
            agg_df, 
            x="Period", 
            y=metric,
    ...
    fig_box.update_yaxes(showgrid=True, gridwidth=1, gridcolor='rgba(255,255,255,0.1)')
    st.plotly_chart(fig_box, use_container_width=True)
    
    # Grouped bar comparison by source
    st.markdown("### 📊 Average Satisfaction by Source & Verification Status")
    comparison_df = filtered_df.groupby(["Source", "Verified"])["Satisfaction"].mean().reset_index()
    comparison_df["Verified"] = comparison_df["Verified"].map({True: "Verified", False: "Unverified"})
    ...
    fig_grouped.update_yaxes(showgrid=True, gridwidth=1, gridcolor='rgba(255,255,255,0.1)')
    ...
    pivot_df = filtered_df.pivot_table(
        values=["Satisfaction", "Correlation"],
        index="Topic",
        aggfunc="mean"
    )
    ...
    fig_heatmap = go.Figure(data=go.Heatmap(
        ...
        texttemplate="%{text}",
        textfont={"size": 10},
        hovertemplate="Topic 1: %{x}<br>Topic 2: %{y}<br>Correlation: %{z:.2f}<extra></extra>"
    ))
...
# Footer
st.markdown("---")
st.caption(f"Visualizing {len(filtered_df)} records • Updated: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
